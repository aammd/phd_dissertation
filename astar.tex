...
\caption[Most species pairs differ from the null expectation]{Most species pairs differ from the null expectation. Histograms show the distribtion of pairwise test statistics ()\(T_{b_{ij}}\)) values for both patch size threshold (\(A^{*}\)) and strength of patch size preference (\(x\)). Each observed value is compared against the null distribution for that species pair (999 replications). Values to the left of the dashed line are \(p < 0.05\).}
\label{fig:pvalhist}
\end{figure}
